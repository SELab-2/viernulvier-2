...
class TestSearchableMultiSelectFilter(TestCase):
    ...
    def test_expected_parameters_include_value_search_and_open(self):
        ...
        self.assertEqual(
            filter_instance.expected_parameters(),
            ["code", "code_q", "code_open"],
        )

    def test_init_pops_custom_search_and_open_params_from_admin_params(self):
        filter_instance, params = self._build_filter("code_q=n&code_open=1&code=nl")
        self.assertNotIn(filter_instance.search_param, params)
        self.assertNotIn(filter_instance.open_param, params)
    ...
    def test_clear_search_url_removes_only_search_state(self):
        ...
        self.assertEqual(filter_instance.clear_search_url, "?code=nl&o=3")

    def test_reset_url_removes_filter_selection_and_search_state(self):
        filter_instance, _ = self._build_filter("code=nl&code_q=nl&code_open=1&o=3")
        self.assertEqual(filter_instance.reset_url, "?o=3")
    ...
    def test_lookups_applies_search_branch(self):
        ...
        self.assertEqual(lookups, [("en", "EN"), ("nl", "NL")])

    def test_value_returns_selected_values_list(self):
        filter_instance, _ = self._build_filter("code=nl&code=en")
        self.assertEqual(filter_instance.value(), ["nl", "en"])

    def test_base_get_option_queryset_raises_not_implemented(self):
