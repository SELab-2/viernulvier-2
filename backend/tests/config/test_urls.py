...
class URLTests(TestCase):
    ...
    def test_health_url_resolves(self) -> None:
        ...
        assert resolver.func == health

    def test_health_endpoint_returns_response(self) -> None:
    ...
    def test_api_urls_included(self) -> None:
        """API route exists (basic check)"""
        response = self.client.get("/api/v1/")
        assert response.status_code != 404

    def test_debug_toolbar_urls_not_included_in_production(self) -> None:
    ...
    def test_debug_urls_block_included(self) -> None:
        ...
        assert "__debug__" in url_patterns

    @override_settings(DEBUG=True, MEDIA_URL="/media/", MEDIA_ROOT=tempfile.gettempdir())
    def test_media_urls_included_in_debug(self) -> None:
        ...
        assert media_pattern_found, "Media pattern should be in urlpatterns when DEBUG=True"

    @override_settings(DEBUG=False)
    def test_media_urls_not_included_in_production(self) -> None:
        ...
        url_patterns = get_resolver().url_patterns
        media_pattern_found = any(hasattr(p, "pattern") and "media/" in str(p.pattern) for p in url_patterns)
        assert not media_pattern_found, "Media pattern should NOT be in urlpatterns when DEBUG=False"
