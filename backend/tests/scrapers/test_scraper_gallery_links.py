...
def test_sync_media_item_gallery_links_updates_gallery_and_position(monkeypatch) -> None:
    ...
    assert changed == 3
    assert item_1.gallery_id == gallery_a.pk
    assert item_1.position == 0
    assert item_2.gallery_id == gallery_a.pk
    assert item_2.position == 1
    assert item_3.gallery_id == gallery_b.pk
    assert item_3.position == 0
    assert links == [
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_clears_stale_links(monkeypatch) -> None:
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_dry_run_does_not_write(monkeypatch) -> None:
    gallery = MediaGalleryFactory.create(external_id="/api/v1/media/galleries/1")
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_fetch_failure_marks_log_failed(monkeypatch) -> None:
    monkeypatch.setattr(
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_empty_payload_creates_success_log(monkeypatch) -> None:
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_dry_run_records_link_errors(monkeypatch) -> None:
    gallery = MediaGalleryFactory.create(external_id="/api/v1/media/galleries/1")
    payload = [
        "not-a-dict",
        {"items": []},
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_partial_success_with_changes_and_errors(monkeypatch) -> None:
    gallery = MediaGalleryFactory.create(external_id="/api/v1/media/galleries/1")
...
@pytest.mark.django_db
def test_sync_media_item_gallery_links_atomic_exception_is_logged_and_reraised(monkeypatch) -> None:
    MediaGalleryFactory.create(external_id="/api/v1/media/galleries/1")
