...
class TestLocationAdminFunctional(TestCase):
	...
	def test_location_translation_add(self):
		...
		self.assertEqual(response.status_code, 200)
		self.assertTrue(
			LocationTranslation.objects.filter(name="Venue Added", language=new_language).exists()
		)
	...
	def test_space_translation_add(self):
		...
		self.assertEqual(response.status_code, 200)
		self.assertTrue(
			SpaceTranslation.objects.filter(name="Room B", language=new_language).exists()
		)
	...
	def test_hall_translation_add(self):
		...
		self.assertEqual(response.status_code, 200)
		self.assertTrue(
			HallTranslation.objects.filter(name="Hall X", language=new_language).exists()
		)
