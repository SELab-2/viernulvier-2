...
class TestProductionAdminFilters(TestCase):
    def setUp(self):
        ...
        TagTranslationFactory(
            tag=self.tag_theater, language=self.language, name="Theater"
        )
        TagTranslationFactory(
            tag=self.tag_family, language=self.language, name="Familie"
        )
        ...
        GenreTranslationFactory(
            genre=self.genre_dance, language=self.language, name="Dans"
        )
        GenreTranslationFactory(
            genre=self.genre_jazz, language=self.language, name="Jazz"
        )
        ...
        ProductionTranslationFactory(
            production=self.production_1, language=self.language, artist_name="John Doe"
        )
        ProductionTranslationFactory(
        ...
        ProductionTranslationFactory(
            production=self.production_3,
            language=LanguageFactory(code="fr", name="French"),
    ...
    def test_tag_filter_uses_and_semantics_for_multiple_selected_tags(self):
        ...
        queryset = filter_instance.queryset(
            filter_instance.request, Production.objects.order_by("id")
        )
        self.assertEqual(
            list(queryset.values_list("id", flat=True)), [self.production_1.id]
        )
    ...
    def test_tag_filter_search_includes_selected_value_outside_search_result(self):
        ...
        option_values = {
            value
            for value, _ in filter_instance.lookups(filter_instance.request, self.admin)
        }
    ...
    def test_tag_filter_uses_non_empty_label_when_type_is_blank(self):
        ...
        TagTranslationFactory(
            tag=tag_without_type, language=self.language, name="Zonder type"
        )
