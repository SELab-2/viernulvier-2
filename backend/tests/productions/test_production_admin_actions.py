...
class TestProductionAdminActions(TestCase):
    ...
    def test_add_tag_action_apply_adds_selected_tag_to_all_selected_productions(self) -> None:
        ...
        assert Production.tags.through.objects.filter(tag_id=tag.pk).count() == 2

    def test_add_genre_action_apply_skips_existing_links_and_sets_next_position(self) -> None:
        ...
        assert response is None
        assert ProductionGenre.objects.filter(production=self.production_1, genre=target_genre).count() == 1

        created_link = ProductionGenre.objects.get(production=self.production_2, genre=target_genre)
        assert created_link.position == 5

    def test_add_tag_action_initial_step_returns_two_step_page(self) -> None:
