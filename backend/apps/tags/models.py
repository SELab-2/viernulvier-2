...
class Tag(BaseModel):
    ...
    url = models.URLField(blank=True, db_comment="The URL of the tag, if it exists.")
    ...
    source_type = models.CharField(
        max_length=100, blank=True, db_comment="Type of the source"
    )

    is_external = models.BooleanField(
        default=False, db_comment="Whether this tag originates from an external system"
    )

    is_enabled = models.BooleanField(
        default=True, db_comment="Whether the tag is enabled"
    )

    type = models.CharField(
        max_length=100, blank=True, db_comment="Type/category of the tag"
    )
...
class TagTranslation(BaseModel):
    ...
    name = models.CharField(
        max_length=255, db_comment="The name of the tag in the specified language."
    )

    short_description = models.TextField(
        blank=True, null=True, db_comment="Short description of the tag"
    )

    url_title = models.CharField(
        max_length=255, blank=True, db_comment="URL title in the specified language"
    )
