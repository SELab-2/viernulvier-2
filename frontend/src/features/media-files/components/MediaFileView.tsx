...
const MediaFileView = ({ mediaFiles, layout = 'list' }: MediaFileViewProps) => {
  ...
  return (
    <>
      {/* CollectionView handles layout switching (list/grid) and
          mobile override (always grid on small screens) internally. */}
      <CollectionView
        items={mediaFiles}
        layout={layout}
        getKey={(file) => file.id}
        renderListItem={(file) => (
          <MediaFileListCard mediaFile={file} onOpen={() => handleOpen(file)} />
        )}
        renderGridItem={(file) => (
          <MediaFileGridCard mediaFile={file} onOpen={() => handleOpen(file)} />
        )}
      />
